...
const PAGE_MAP = [
{ match: /^\/(?:en|hi|bn|te|mr|ta)?\/?$/, image: '/content/uploads/Golden_Key_1_6bb951bf86.png', id: 'KEY_HUNT:1', type_ball: 'golden' },
{ match: /(^|\/)casino\/live-casino\/?$/, image: '/content/uploads/Golden_Key_1_6bb951bf86.png', id: 'KEY_HUNT:2', type_ball: 'golden' },
{ match: /(^|\/)casino\/slots\/?$/, image: '/content/uploads/Golden_Key_1_6bb951bf86.png', id: 'KEY_HUNT:3', type_ball: 'golden' },
{ match: /(^|\/)casino\/instant-games\/?$/, image: '/content/uploads/Golden_Key_1_6bb951bf86.png', id: 'KEY_HUNT:4', type_ball: 'golden' },
{ match: /(^|\/)my-bets\/open\/?$/, image: '/content/uploads/Golden_Key_1_6bb951bf86.png', id: 'KEY_HUNT:5', type_ball: 'golden' },
{ match: /(^|\/)search\/?$/, image: '/content/uploads/Golden_Key_1_6bb951bf86.png', id: 'KEY_HUNT:6', type_ball: 'golden' }
];
...
function getUserTags() {
  ...
  try {
    ...
    if (Array.isArray(stored)) {
      tags = stored.filter(tag => tag.startsWith('KEY_HUNT'));
    }
  } catch (e) {
    tags = [];
  }
...
_smartico.on('props_change', function(props) {
  if (props.core_public_tags !== undefined) {
    ...
    const smartTags = Array.isArray(props.core_public_tags)
      ? props.core_public_tags.filter(tag => tag.startsWith('KEY_HUNT'))
      : [];
...
requestAnimationFrame(() => {el.style.opacity = 1;
 ...
 window.analytics.push({
  ...
  data: {
    entity: "widget_item",
    item_id: config.id,
    item_name: "hunt_key_view",
    item_type: config.type_ball,
    page_id: location.pathname,
    page_name: location.pathname,
    page_type: location.pathname,
    widget_name: "hunt_key"
  }
});

setTimeout(() => {
const tags = getUserTags() || [];
const isFirstBallEver = !tags.some(tag => tag.startsWith('KEY_HUNT'));
...
!['golden'].includes(config.type_ball) ||
...
<div class="tooltip-title">
    You're Invited to the Golden Hour Tournament
  </div>

  <div class="tooltip-text">
    Collect 3 Golden Keys hidden on the site to claim your VIP entry.
  </div>
<div class="tooltip-actions">
<span class="skip-tooltip">Skip</span>
<button class="tooltip-btn">COLLECT</button>
</div>
</div>
</div>
`;
...
exploreBtn.addEventListener('click', () => {
  ...
  handleBallAction(config, targetEl, () => {
    _smartico.dp('dp:gf_tournaments&id=5315');
  });
});
...
if (!tags.includes(config.id)) {
 ...
 window.analytics.push({
  schema: 'iglu:com.psp/widget_platform_click/jsonschema/5-0-0',
  data: {
    click_type: "hunt_key_click",
    entity: "widget_item",
    item_id: config.id,
    item_name: "hunt_key_click",
    item_type: config.type_ball,
    page_id: location.pathname,
    page_name: location.pathname,
    page_type: location.pathname,
  ...
  _smartico.action('GOLDEN_KEY_ACTION', {

(function () {
    ...
    function syncSmarticoUser(force = false) {
        ...
        if (!token) {
            ...
            // удаляем сохраненные данные Smartico
            localStorage.removeItem('smartico_skin');
            localStorage.removeItem('smartico_control');

            console.log('[Smartico] User logged out, localStorage cleared');
            return;
        }
    ...
    function bindSmarticoEvents() {
        ...
        // проверка текущих props сразу после подписки
        if (typeof window._smartico.getUserProps === 'function') {

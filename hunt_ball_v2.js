...
{ match: /(^|\/)horseracing\/live|horseracing\/?$/, image: '/content/uploads/Red_Ball_3705195b14.webp', id: 'LEMON_HUNT:8', type_ball: 'redball' },
...
function getUserTags() {
  ...
  try {
    // Читаем теги из localStorage
    const stored = JSON.parse(localStorage.getItem('smartico_tags') || '[]');
    if (Array.isArray(stored)) {
      // Фильтруем только теги, начинающиеся с 'LEMON_HUNT'
  ...
  // Получаем теги из API, если они существуют
  ...
  if (Array.isArray(smartTags) && smartTags.length) {
    ...
    // Сохраняем актуальные теги в localStorage
    try {
      localStorage.setItem('smartico_tags', JSON.stringify(tags));
    } catch (e) {}
  }

  return tags; // Возвращаем только правильные теги
}
...
console.log('Ball clicked', config.id);

_smartico.updatePublicTags('add', [config.id]);
_smartico.action('lemon_hunt_action_week_1', { custom_location: config.type_ball });

// Сохраняем локально сразу после клика
const tags = getUserTags();
tags.push(config.id);
localStorage.setItem('smartico_tags', JSON.stringify(tags));
}, { once: true });
...
targetEl.addEventListener('click', () => {closeTooltip(); handleBallAction(config, targetEl); _smartico.dp('dp:gf_section&id=1398');}, { once: true });
...
history.pushState = function () { pushState.apply(history, arguments); removeTooltip(); setTimeout(render, CONFIG.reRenderDelay); };
window.addEventListener('popstate', () => {removeTooltip();setTimeout(render, CONFIG.reRenderDelay);});
}
